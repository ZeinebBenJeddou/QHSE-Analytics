...
public class QualityReportBuilder {
    ...
    public ImportQualityReport build(List<KpiRawDataDTO> rawData, boolean allowPartialImport) {
        ...
        List<com.QHSEAnalytics.dto.response.RejectedReasonSummary> rejectedReasons = rejectedReasonsMap.entrySet().stream()
                .map(e -> new com.QHSEAnalytics.dto.response.RejectedReasonSummary(e.getKey(), e.getValue()))
                .toList();
