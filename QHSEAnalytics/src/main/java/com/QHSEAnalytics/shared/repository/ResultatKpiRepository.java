...
public interface ResultatKpiRepository extends JpaRepository<ResultatKpi, Long> {
    ...
    @Query("select r.kpi.id as kpiId, r.variationAbsolue as variation " +
           "from ResultatKpi r " +
           "where r.kpi.id in :kpiIds and r.variationAbsolue is not null " +
           "order by r.createdAt desc")
    List<VariationHistoryProjection> findVariationHistoryByKpiIds(@Param("kpiIds") List<Long> kpiIds);

    interface VariationHistoryProjection {
        Long getKpiId();
        Double getVariation();
    }
    ...
    long countByImportSessionIdAndNiveauVariation(Long importSessionId, com.QHSEAnalytics.shared.enums.NiveauVariation niveau);
    ...
    List<ImportSessionCountView> countByImportSessionIdsAndNiveauVariation(@Param("importSessionIds") List<Long> importSessionIds, @Param("niveau") com.QHSEAnalytics.shared.enums.NiveauVariation niveau);

    long countByNiveauVariation(com.QHSEAnalytics.shared.enums.NiveauVariation niveau);

    @Query("select r from ResultatKpi r join fetch r.kpi k join fetch k.categorieKpi where r.niveauVariation = :niveau")
    List<ResultatKpi> findByNiveauVariationWithKpi(@Param("niveau") com.QHSEAnalytics.shared.enums.NiveauVariation niveau);
