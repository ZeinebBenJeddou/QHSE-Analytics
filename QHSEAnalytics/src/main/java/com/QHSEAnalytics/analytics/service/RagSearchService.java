...
public class RagSearchService {
    ...
    private List<RagKnowledge> vectorSearch(String vectorLiteral, int topK, String category, double threshold) {
        if (category != null && !category.isBlank()) {
            return jdbcTemplate.query(
                "SELECT id, kpi_name, chunk_type, definition, thresholds, category FROM rag_knowledge " +
    ...
    private List<RagKnowledge> keywordSearch(String query, int topK) {
        ...
        StringBuilder sql = new StringBuilder(
            "SELECT id, kpi_name, chunk_type, definition, thresholds, category FROM rag_knowledge WHERE ");
